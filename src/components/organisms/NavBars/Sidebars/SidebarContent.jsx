...
const SidebarContent = () => {
  ...
  return (
    <UlContainerCenter className={`text-neutral-400 cursor-pointer`}>
      {items.map((item) => {
        ...
        return (
          <div key={item.id} className="w-full">
            ...
            <div
              className={`w-full overflow-y-scroll overflow-x-hidden transition-all border-b border-neutral-700
              ${isOpen ? "max-h-80 opacity-100 duration-500" : "max-h-0 opacity-0 duration-300"}
              `}
            >
              <Link to="/#books-section">{item.content}</Link>
            </div>
          </div>
        );
      })}
